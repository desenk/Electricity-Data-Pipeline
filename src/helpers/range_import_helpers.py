...
def iter_data_extract(
    func_name=temperature, start_date="2020-03-24", end_date="2020-03-25", weekly=False
):
    ...
    while rel_start_date <= end_date_dt:
        ...
        for x in [func_name]:
            df1 = x(
                rel_start_date.strftime(format_str), rel_end_date.strftime(format_str)
            )
...
def iter_data_extract_without_helper(
    report_name="TEMP", start_date="2020-03-24", end_date="2020-03-25", weekly=False
):
    ...
    while rel_start_date <= end_date_dt:
        ...
        for x in [extract_data]:
            df1 = x(
                report_name,
                rel_start_date.strftime(format_str),
                rel_end_date.strftime(format_str),
            )
        df = pd.concat([df, df1], ignore_index=True)
        rel_start_date = rel_start_date + delta
    return df
...
def extract_data_range_with_BMRS_label(
    report_name="TEMP",
    start_date="2020-03-24",
    end_date="2020-03-25",
    save_to_csv=False,
